...
paper/         main.tex, references.bib, and figures/make_figures.py
results/       committed run artifacts
data/          local only, gitignored (see data/README.md)
```

Everything under `paper/figures/` is regenerated by `make_figures.py` from the
committed artifacts in `results/`; no figure is hand-placed.
...
This work grew out of an earlier collaboration with Ethan Cao and Patrick Ho, who
are not involved in the current version. That earlier draft reported layer-wise
geometric specialization in MeshXL attention. It does not survive the recency
control documented above: once sequence position is accounted for, the early
layers it described carry no spatial selectivity at all. Several of its figures
also reported quantities for which no computation existed. Rather than patch
those claims quietly, the present version re-derives everything from scratch
under preregistered rules, and the reasoning is visible in
[docs/](docs/) and in the commit history.

MeshXL is by Chen et al. (NeurIPS 2024); the chair fine-tune used here is the
authors' own public checkpoint. ShapeNet is distributed under a research-only
